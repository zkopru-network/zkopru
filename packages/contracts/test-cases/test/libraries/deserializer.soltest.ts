...
import { Block, Header, Body, serializeHeader, serializeBody } from "~core";
    ...
    it("should have correct tx outflow data", async () => {
      for (let txIndex = 0; txIndex < body.txs.length; txIndex += 1) {
        for (
          ...
          compare(txOutflow.to, data ? data.to : 0);
          compare(txOutflow.eth, data ? data.eth : 0);
          compare(txOutflow.token, data ? data.tokenAddr : 0);
          compare(txOutflow.amount, data ? data.erc20Amount : 0);
          compare(txOutflow.nft, data ? data.nft : 0);
          compare(txOutflow.fee, data ? data.fee : 0);
        }
      }
    });
