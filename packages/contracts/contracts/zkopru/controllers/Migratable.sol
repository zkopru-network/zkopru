...
contract Migratable is Storage {
    ...
    function migrateTo(
        ...
        MassMigration memory migration =
            Deserializer.massMigrationFromCalldataAt(1);
            ...
            )
        {
            ...
            for (uint256 i = 0; i < migration.erc20.length; i++) {
                IERC20(migration.erc20[i].addr).transfer(
                    to,
                    migration.erc20[i].amount
                );
            }
            // send ERC721
    ...
    function acceptMigration(
        ...
        require(
            Storage.allowedMigrants[msg.sender],
            "Not an allowed departure"
        );
