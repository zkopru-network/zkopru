...
export default class UpdateUrl extends App {
  static code = AppMenu.UPDATE_URL
  ...
  async run(context: Context): Promise<{ context: Context; next: number }> {
    ...
    const myUrl = await auction.methods.coordinatorUrls(this.base.context.account.address).call()
    this.print(chalk.blue(`Current url: ${myUrl}`))
    const { url } = await this.ask({
      type: 'text',
      name: 'url',
      initial: '',
      message: 'Enter your URLs formatted as "host:port". Multiple values can be comma separated.',
    })
    ...
    // send tx
    const tx = this.base.layer1().sendExternalTx(
