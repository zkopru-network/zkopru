...
COPY ./packages/contracts/testing_migrations /proj/migrations
COPY ./packages/contracts/truffle-config.js /proj/truffle-config.js
RUN truffle compile
EXPOSE 5000
RUN ganache-cli --db=/data -i 20200406 -p 5000 --deterministic --host 0.0.0.0 & sleep 5 && truffle migrate --network insidedocker
